...
const POLITICS_COUNTRIES = [
  { code: 'us', label: 'US', fullName: 'United States' },
  { code: 'ca', label: 'Canada', fullName: 'Canada' },
  { code: 'uk', label: 'UK', fullName: 'United Kingdom' },
  { code: 'au', label: 'Australia', fullName: 'Australia' },
];

const TOTAL_STEPS = CATEGORIES.length + POLITICS_COUNTRIES.length;
...
async function generatePoliticsQuiz(country: { code: string; label: string; fullName: string }) {
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: 'user',
        content: 'Search for the latest ' + country.fullName + ' politics news from today or in the last 24 hours. Focus specifically on ' + country.fullName + ' domestic politics, government, elections, legislation, and political figures.\n\nCreate exactly 6 quiz questions based on REAL current ' + country.fullName + ' political news stories. Focus on MAJOR stories that most people following ' + country.fullName + ' politics would have heard about.\n\nRULES:\n- Questions about verifiable public facts, NOT opinions\n- Write everything in YOUR OWN words\n- Do NOT name any news outlet\n- Keep explanations to 1 sentence\n- Add a "perspective" field: one sentence on how this story is viewed differently by different political sides or internationally\n- Add a "context" field: 2-3 sentences of background\n- The "headline" must NEVER reveal the correct answer\n- Do NOT include any HTML tags, citation tags, or markup\n\nReturn ONLY a JSON array. Each item:\n{"question":"...","options":["A","B","C","D"],"correct":0,"explanation":"1 sentence","headline":"Short topic label","perspective":"1 sentence","context":"2-3 sentences"}\n\nKeep the ENTIRE response concise. No markdown, no backticks, no HTML tags.',
      }],
    }),
  });
...
export async function GET(request: Request) {
  ...
  if (step < CATEGORIES.length) {
    ...
    const countryIdx = step - CATEGORIES.length;
    const country = POLITICS_COUNTRIES[countryIdx];
    try {
      const questions = await generatePoliticsQuiz(country);
