...
export default function Home() {
  ...
  const fetchQuiz = async (cat: any): Promise<any> => {
    let url = '/api/quiz?category=' + cat.label;
    if (cat.hasCountries) {
  ...
  const getDisplayLabel = () => {
    if (!category) return '';
    if (category.hasCountries) {
      const country = POLITICS_COUNTRIES.find(c => c.code === selectedCountry);
      return country ? country.flag + ' ' + country.label + ' Politics' : category.label;
    }
        ...
        {screen === 'home' && (
          <div>
            <div className="text-center pt-12 pb-5 border-b-2 border-gray-900">
              ...
              <p className="font-sans" style={{fontFamily:"Georgia,serif",fontStyle:"italic",fontSize:"14px",color:"#737373",animation:"logoFadeUp 0.4s ease 1s forwards",opacity:0}}>play the news.</p>
            </div>
            ...
            {revealed && (
              <div className="mt-6">
                <div className="animate-slide-up bg-gray-50 rounded-lg p-4 mb-3 border border-gray-200">
                  <p className={'text-xs font-semibold uppercase tracking-wide mb-1 font-sans ' + (timedOut ? 'text-amber-600' : selected === questions[currentQ].correct ? 'text-green-600' : 'text-red-600')}>
          ...
          <div className="pt-12">
            {roundCorrect === 0 ? (
              <div className="text-center pb-7 border-b-2 border-gray-900 mb-8">
                ...
                <p className="text-sm text-gray-400 font-sans">The news moves fast — come back tomorrow and show it who is boss.</p>
              </div>
            ) : (
              <div className="text-center pb-7 border-b-2 border-gray-900 mb-8">
